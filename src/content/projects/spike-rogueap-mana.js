...
## Results
Successfully embedded offensive network infrastructure inside an inconspicuous physical object, proving the viability of low-cost drop devices for physical penetration testing and social engineering labs.

Engineered a custom captive portal hosted entirely on the ESP32's flash memory. The portal actively intercepts client web traffic and serves custom HTML payloads to test credential harvesting and HTTP redirection flows.

Integrated and programmed pulsing LED eyes into the porcupine's face, providing a real-time, screenless indicator of AP status and successful client connections.

Demonstrated that a low-voltage microcontroller can independently handle SSID broadcasting, DHCP assignment, and DNS hijacking without needing a full Linux OS or external power brick.
...
While the 2 NRF-24 Antennas look really funny sticking out of the porcupine, they dont actually add any range or functionality to the device- in THIS configuratiopn. They work for other firmwares I have toyed with that use them in ConstCarrier mode in a controlled enviornment. 

While the ESP32 is a powerhouse for IoT, its Wi-Fi stack and memory are constrained. It requires highly optimized code to handle simultaneous AP broadcasting, DNS spoofing, and web-serving without crashing when multiple clients connect.
